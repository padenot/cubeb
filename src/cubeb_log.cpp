...
/** The maximum number of log messages that can be queued before dropping messages. */
const size_t CUBEB_LOG_MESSAGE_QUEUE_DEPTH = 40;
/** Number of milliseconds to wait before dequeuing log messages. */
const std::chrono::milliseconds CUBEB_LOG_BATCH_PRINT_INTERVAL_MS = std::chrono::milliseconds(10);

/**
  * This wraps a 256 bytes buffer, that represents
  * a log message, that must be null-terminated.
...
static_assert(std::is_trivially_copyable<cubeb_log_message>::value,
              "A log message must be trivially copyable to work with the lock-free queue");
...
private:
  ...
  /** This is quite a big data structure, but is only
    * instantiated if the asynchronous logger is used.*/
