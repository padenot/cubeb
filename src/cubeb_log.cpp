...
public:
  ...
  void run()
  {
    ...
    logging_thread = std::thread([this]() {
      ...
      while (!shutdown_thread) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(CUBEB_LOG_BATCH_PRINT_INTERVAL_MS));
      }
  ...
  void start() {
    msg_queue.reset(new lock_free_queue<cubeb_log_message>(CUBEB_LOG_MESSAGE_QUEUE_DEPTH));
...
void cubeb_noop_log_callback(char const * /* fmt */, ...) { }
